...
class CasinoGames {
    constructor() {
        ...
        // Ждем, пока telegramCasino будет доступен
        if (window.telegramCasino) {
            this.initGames();
        } else {
            console.warn("⚠️ telegramCasino не загружен, ждем...");
    ...
    setupMainButtons() {
        const buttons = {
            ...
            transferBtn: () => {
                if (window.telegramCasino) {
                    window.telegramCasino.showScreen("transfer");
                } else {
                    console.error("telegramCasino не загружен");
                }
            },
            historyBtn: () => {
                if (window.telegramCasino) {
            ...
            gamesBtn: () => {
                if (window.telegramCasino) {
                    window.telegramCasino.showScreen("games");
                } else {
                    console.error("telegramCasino не загружен");
                }
            },
            adminBtn: () => {
                if (window.telegramCasino) {
                    window.telegramCasino.showScreen("admin");
                } else {
                    console.error("telegramCasino не загружен");
                }
            },
    ...
    async playDice() {
        ...
        setTimeout(async () => {
            ...
            if (saved) {
                if (winAmount > 0) {
                    telegramCasino.showNotification(
                        `🎲 Выиграно ${winAmount} монет!`,
                        "success",
                    );
                } else {
                    telegramCasino.showNotification(
                        `🎲 Проигрыш ${this.currentBet} монет`,
                        "info",
                    );
                }
            }
