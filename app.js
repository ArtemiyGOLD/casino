...
window.register = async function () {
    ...
    const passwordConfirm = document
        ...
        .value.trim();
    const name =
        document.getElementById("registerName").value.trim() || username;
    ...
    try {
        const user = await window.supabaseAuth.register(
            username,
            password,
            name,
        );
        showNotification(
            `Регистрация успешна! Добро пожаловать, ${user.name}! 🎉`,
            "success",
        );

        // Автоматически входим
        const loggedInUser = await window.supabaseAuth.login(
            username,
            password,
        );
...
window.makeTransfer = async function () {
    ...
    try {
        ...
        if (result.success) {
            showNotification(
                `✅ Успешно переведено ${amount} монет пользователю ${result.toUser}`,
                "success",
            );
...
window.loadAllUsers = async function () {
    try {
        ...
        html +=
            "<div>ID</div><div>Логин</div><div>Имя</div><div>Баланс</div><div>Роль</div><div>Действия</div>";
        html += "</div>";
...
window.confirmDeleteUser = function (userId) {
    if (
        confirm(
            "Вы уверены что хотите удалить этого пользователя? Все его данные будут удалены.",
        )
    ) {
        deleteUser(userId);
    }
...
window.adminAddCoins = async function () {
    ...
    try {
        ...
        showNotification(
            `✅ Добавлено ${amount} монет пользователю ${result.username}`,
            "success",
        );
...
async function loadGameHistory() {
    try {
        ...
        const games = await window.supabaseAuth.getGameHistory(
            currentUser.id,
            10,
        );
        ...
        if (!games || games.length === 0) {
            historyList.innerHTML =
                '<div class="empty">История игр пуста</div>';
            return;
        }
