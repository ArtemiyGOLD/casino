...
class DiceGame {
    ...
    async roll() {
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                // Показываем результат
                if (winAmount > 0) {
                    showNotification(`🎉 Вы выиграли ${winAmount} монет!`, 'success');
                } else {
                    showNotification(`😔 Вы проиграли ${currentGame.bet} монет`, 'info');
                }
    ...
    showDiceResult(dice1, dice2, sum) {
        ...
        this.diceElement.style.transform = 
            `rotateX(${rotations[dice1].x}deg) rotateY(${rotations[dice1].y}deg)`;
...
class SlotsGame {
    ...
    async spin() {
        ...
        this.reels.forEach((reel, index) => {
            ...
            const randomPosition = Math.floor(Math.random() * this.symbols.length);
        ...
        setTimeout(async () => {
            ...
            try {
                await window.supabaseClient.saveGameResult({
                    ...
                    bet: currentGame.bet,
                    win: winAmount,
                    result: { symbols: results, winMultiplier: winAmount / currentGame.bet }
                });
                ...
                if (winAmount > 0) {
                    showNotification(`🎰 Выигрыш: ${winAmount} монет!`, 'success');
                } else {
